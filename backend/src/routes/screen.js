...
router.get('/device/:deviceId', authMiddleware, async (req, res) => {
  const result = await db.query(
    `SELECT * FROM screen_recordings WHERE device_id = (SELECT id FROM devices WHERE device_id = $1) 
...
router.get('/download/:recordingId', authMiddleware, async (req, res) => {
  const result = await db.query('SELECT minio_key FROM screen_recordings WHERE id = $1', [req.params.recordingId]);
